...
import { Modal, Form, Row, Col, Select, DatePicker, Descriptions } from 'antd'
import dayjs from 'dayjs'
import { START_WORK_TYPE_OPTIONS, CITY_CODE_SEEDS } from '../constants/projectEnums'

/**
 * 编辑落地信息弹窗：与「签约转落地」弹窗同构。
 * - 签约阶段信息只读回显；
 * - 仅可编辑落地阶段 2 个字段（开工开业类型、开工/开业时间）；
 * - 打开时带出已填写的开工信息（优先编辑持久化值，兜底 mock 原始字段）。
 */
export default function LuodiEditModal({ open, projectData, onCancel, onOk }) {
  ...
  useEffect(() => {
    if (open && projectData) {
      ...
      form.setFieldsValue({
        // 带出已填写的开工信息（编辑持久化值优先，mock 原始字段兜底）
  ...
  // label 160px + nowrap：与转落地弹窗一致，长标签不换行
