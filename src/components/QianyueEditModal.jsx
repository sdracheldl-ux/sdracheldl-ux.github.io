...
// 编辑弹窗：与转签约弹窗同构的字段集（在谈19字段 + 签约13字段）
export default function QianyueEditModal({ open, projectData, onCancel, onOk }) {
  ...
  useEffect(() => {
    if (open && projectData) {
      ...
      const initialValues = {
        // 基础信息
        projectCode: clean(g('projectCode', 'districtProjectCode', '区级项目编码', '编号')),
        projectName: clean(g('projectName', '项目名称')),
        responsibleUnits: normalizeResponsibleUnits((() => { const v = g('responsibleUnits', '责任单位'); return v === '-' ? undefined : v })()),
        capitalNature: clean(g('capitalNature', '内外资')),
        sourceRegion: toCascader(clean(g('sourceRegion', 'sourceArea', '来源地'))),
        industryType: clean(g('industryType', '产业类别')),
        industryCategory: toCascader(g('industryCategory', '行业类别')),
        secondaryIndustryCategory: toCascader(clean(g('secondaryIndustryCategory', '次要行业类别'))),
        projectDescription: clean(g('projectDescription', 'projectDesc', '项目简介')),

        // 在谈阶段信息
        projectCategory: clean(g('projectCategory', '项目分类')),
        investAmount: g('investAmount', '计划投资总额(亿元)', '投资金额(亿元)', '投资金额（亿元）'),
        needInvestAmount: g('needInvestAmount', '需投资金额(亿元)'),
        investorEntity: clean(g('investorEntity', '投资主体')),
        dockingDate: toDayjs(clean(g('dockingDate', 'contactTime', '对接时间'))),
        enterpriseCategory: clean(g('enterpriseCategory', '企业类别')),
        isStock: g('isStock', '是否为存量企业', '是否存量企业') || '否',
        registeredCapitalAmount: g('registeredCapitalAmount', '注册资本(亿元)'),
        ...
        merchantType: clean(g('merchantType', '招商类型')),
        merchantTypeDesc: clean(g('merchantTypeDesc', '招商类型说明')),
        isZheshang: g('isZheshang', '是否浙商') || '否',
        landSituation: clean(g('landSituation', '用地情况')),
        isHqEconomy: g('isHqEconomy', '是否总部经济') || '否',
        hqEconomyLevel1: clean(g('hqEconomyLevel1', '总部经济类型一级')),
        hqEconomyLevel2: clean(g('hqEconomyLevel2', '总部经济类型二级')),
        recordAmount: g('recordAmount', '备案证金额(亿元)'),
        fixedInvestAmount: g('fixedInvestAmount', '固投金额(亿元)'),
        agreementSignDate: toDayjs(clean(g('agreementSignDate', 'signDate', '协议签订时间'))),
        agreementType: clean(g('agreementType', '协议类型')),
        isRegister: g('isRegister', 'isRegistered', '是否注册', '是否已注册') || '否',
        registerDate: toDayjs(clean(g('registerDate', '注册时间'))),
        registerCapitalAmount: g('registerCapitalAmount', 'registeredCapital', '注册注册资本'),
        registerCapitalUnit: clean(g('registerCapitalUnit', 'capitalUnit', '注册资本单位')) || '万元',
        registerCompanyName: clean(g('registerCompanyName', 'registeredCompany', 'registerCompanyNameCn', '注册公司名称', '注册公司')),
        signSubject: clean(g('signSubject', 'signEntity', '签约主体(洽谈主体)', '签约主体')),
      }
